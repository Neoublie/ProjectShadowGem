...
public class psBlock_DisplayCase extends Block {
	...
	private static final VoxelShape SHAPE_N = Stream.of(
			...
			Block.makeCuboidShape(3, 10, 12, 13, 11, 13)
	...
	private static final VoxelShape SHAPE_E = Stream.of(
			...
			Block.makeCuboidShape(3, 0, 3, 13, 1, 13),
			Block.makeCuboidShape(6, 1, 6, 10, 9, 10),
			Block.makeCuboidShape(3, 10, 3, 4, 11, 13)
			).reduce((v1, v2) -> {return VoxelShapes.combineAndSimplify(v1, v2, IBooleanFunction.OR);}).get();
	...
	public TileEntity createTileEntity(BlockState state, IBlockReader level) {
		return TileEntityTypeInit.DISPLAY_CASE_TILE_ENTITY_TYPE.get().create();
	}
	...
	public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player,
		...
		if (!worldIn.isRemote()) {
			...
			if (te instanceof psTile_DisplayCase) {
				NetworkHooks.openGui((ServerPlayerEntity) player, (psTile_DisplayCase) te, pos);
			}
		}
